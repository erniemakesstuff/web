...
export class MedicineListComponent implements OnInit, OnDestroy {
  columnDefinitions = [
    ...
    { def: 'stock', label: 'Stock', type: 'number', visible: true },
  ...
  private updateRecord(updatedRecord: MedicineList) {
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => ({
      ID: x.id,
      'Medicine Number': x.m_no,
      'Medicine Name': x.m_name,
      Category: x.category,
      Company: x.company,
      'Purchase Date': formatDate(new Date(x.p_date), 'yyyy-MM-dd', 'en') || '',
      Price: x.price,
      'Expiry Date': formatDate(new Date(x.e_date), 'yyyy-MM-dd', 'en') || '',
      Stock: x.stock,
    }));

    TableExportUtil.exportToExcel(exportData, 'medicine_data_export');
  }

  onContextMenu(event: MouseEvent, item: MedicineList) {
