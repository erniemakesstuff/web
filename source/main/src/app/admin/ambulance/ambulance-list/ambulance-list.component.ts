...
export class AmbulanceListComponent implements OnInit, OnDestroy {
  columnDefinitions = [
    ...
    { def: 'actions', label: 'Actions', type: 'actionBtn', visible: true }, // If applicable
  ];
  ...
  private updateRecord(updatedRecord: AmbulanceList) {
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => ({
      ...
      'Driver License Number': x.driver_license_no,
      'Driver Number': x.driver_no,
      'Vehicle Type': x.vehicle_type,
      Notes: x.note,
    }));

    TableExportUtil.exportToExcel(exportData, 'vehicle_data_export');
  }

  onContextMenu(event: MouseEvent, item: AmbulanceList) {
