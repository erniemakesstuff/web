...
import { DeathFormComponent } from './dialog/form-dialog/form-dialog.component';
import { DeathDeleteComponent } from './dialog/delete/delete.component';
...
export class DeathComponent implements OnInit, OnDestroy {
  columnDefinitions = [
    ...
    { def: 'death_date', label: 'Death Date', type: 'date', visible: true },
    {
      def: 'guardian_name',
      label: 'Guardian Name',
      type: 'text',
  ...
  dataSource = new MatTableDataSource<Death>([]);
  ...
  loadData() {
    this.deathService.getAllDeaths().subscribe({
      next: (data) => {
        ...
        this.refreshTable();
        this.dataSource.filterPredicate = (data: Death, filter: string) =>
  ...
  openDialog(action: 'add' | 'edit', data?: Death) {
    ...
    const dialogRef = this.dialog.open(DeathFormComponent, {
      width: '60vw',
      maxWidth: '100vw',
      data: { death: data, action },
  ...
  private updateRecord(updatedRecord: Death) {
  ...
  deleteItem(row: Death) {
    const dialogRef = this.dialog.open(DeathDeleteComponent, {
      data: row,
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  onContextMenu(event: MouseEvent, item: Death) {
